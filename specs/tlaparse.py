...
def parse_tla_file(workdir, specname):
    ...
    if workdir is not None:
        prepath = f"{workdir}/"
    spec_text_lines = open(f"{prepath}{specname}.tla").readlines()

    xml_ast = f'{prepath}{xml_out_file}'
    tree = ET.parse(xml_ast)
    return TLASpec(tree, spec_text_lines)
    # return tree

if __name__ == "__main__":
    tla_file = "RaftAbstractDynamic"
    my_spec = parse_tla_file("RaftAbstractDynamic", tla_file)
